...
class Analyzer:
    ...
    """
    def __init__(self, df: pd.DataFrame, point: list):
        """
        ...
        if not self._validate_init(df, point):
    ...
    @staticmethod
    def _validate_init(df: pd.DataFrame, point: list) -> bool:
        """
    ...
    @point.setter
    def point(self, point: list) -> None:
        """
        Метод изменения значения координат города.

        Аргумент:
            point: список координат города.
    ...
    def get_differential(self) -> pd.DataFrame:
        ...
        try:
            ...
            self._logger.info("Найден дифференциал временного ряда")
        except:
